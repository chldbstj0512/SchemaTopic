...
def train(model, network_architecture, data_dict, PC, TC, options, test_prior_covars,
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            cost, recon_y, thetas, nl, kld, cl = model.fit(epoch, options, batch_xs, batch_ys, batch_pcs, batch_tcs, eta_bn_prop=eta_bn_prop,
        ...
        if (epoch + 1) % options.eval_step == 0:
            ...
            run_name = '%s_%s_K%s_seed%s' % (options.model, options.dataset, options.n_topic, options.seed)
...
if __name__ == '__main__':
    ...
    parser.add_option('--n_epochs', type=int, default=250)
    parser.add_option('--n_topic_words', default=10, type=int)

    parser.add_option('--lr', type=float, default=0.001)
    parser.add_option('--bs', type=int, default=500, help='Size of minibatches: default=%default')
