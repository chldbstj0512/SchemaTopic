#!/usr/bin/env bash
# 10건 스키마 실행 — 모델만 gpt-5.2 (OpenAI API), 출력은 results/samples_gpt5.2
# 사용: bash scripts/run_10_cases_gpt52.sh
#       (OpenAI 사용 시 GPU 불필요, 순차 실행)
...
OUT_ROOT="${OUT_ROOT:-results/samples_gpt5.2}"
mkdir -p "$OUT_ROOT"
RESULT_TSV="${RESULT_TSV:-troubleshooting/run_result_gpt52.tsv}"
...
echo "=============================================="
echo " 10건 스키마 실행 (model=$MODEL_NAME, out=$OUT_ROOT)"
...
for i in $(seq 0 9); do
  ...
  echo "  실행 중 ... (schema only, $MODEL_NAME)"
  if python3 main.py schema --topic_words_file "$vanilla_path" --out_dir "$out_dir" --model_name "$MODEL_NAME" $keep_flag 2>&1; then
...
# troubleshooting/samples_gpt5.2 에 심볼릭 링크로 모음 (바로 열어보기용)
SAMPLES_LINK_DIR="troubleshooting/samples_gpt5.2"
mkdir -p "$SAMPLES_LINK_DIR"
for name in 20ng_ecrtm_keep dbpedia_plda_seed1 dbpedia_plda_seed2 dbpedia_plda_seed3 dbpedia_nvdm_seed5 r8_nvdm_seed1 r8_nvdm_seed2 r8_nvdm_seed3 r8_nvdm_seed4 r8_nvdm_seed5; do
  ln -sfn "$(pwd)/$OUT_ROOT/$name" "$SAMPLES_LINK_DIR/$name"
done

echo ""
echo "=============================================="
echo " 10건 모두 처리 완료. 결과: $RESULT_TSV"
echo " 출력: $OUT_ROOT  /  바로 열기: $SAMPLES_LINK_DIR"
echo "=============================================="
cat "$RESULT_TSV"
