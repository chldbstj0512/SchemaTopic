#!/bin/bash
#
# SchemaTopic Pipeline - K 고정 (keep) 모드
# K 모드: keep (--keep, 모든 토픽 유지)
#
# NTM 5종 x 데이터 4종 x 시드 5 = 100 실험
#
# 사용법:
#   CUDA_VISIBLE_DEVICES=1 ./run_pipeline_keep.sh           # GPU 1에서 실행
#   CUDA_VISIBLE_DEVICES=1 ./run_pipeline_keep.sh --dry-run # 명령만 출력
#
...
STATUS_FILE="${LOG_DIR}/run_status_keep.txt"
FAILED_FILE="${LOG_DIR}/failed_keep.txt"
mkdir -p "$LOG_DIR"
...
echo "=========================================="
echo "SchemaTopic Pipeline [keep] 실험 시작"
echo "총 ${total}개 실험"
echo "상태: $STATUS_FILE"
...
for model in "${NTM_MODELS[@]}"; do
  for dataset in "${DATASETS[@]}"; do
    for seed in "${SEEDS[@]}"; do
      ...
      log_file="${LOG_DIR}/pipeline_${dataset}_${model}_keep_seed${seed}.log"

      echo ""
      echo "[${current}/${total}] dataset=${dataset} model=${model} k_mode=keep seed=${seed}"
